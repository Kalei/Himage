    ...
    public function save() {
        ...
        if (!empty($this->id)) {
            ...
            foreach ($this->data as $att => $value)
                if ($att != 'id' && $att != 'id_image' && $value)
                    $set[] = " $att = " . $pdo->quote($value) . " ";

            $sql .= implode(",", $set);
            $sql .= " WHERE id_image =" . $this->id;
            $last_id = $this->id;
            try {
                $pdo->exec($sql);
            ...
            $query = "SELECT id_image FROM image ORDER BY id_image DESC LIMIT 0 , 1";
            try {
