...
class UploadHandler {
     ...
     protected $error_messages = array(
         ...
         'connect_error' => 'Connection to BDD fail',
     ...
     function __construct($options = null, $initialize = true, $error_messages = null) {
          $this->options = array(
              ...
              'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')) . '/www/images/photos/',
              'upload_url' => $this->get_full_url() . '/www/images/photos/',
              'user_dirs' => false,
              'mkdir_mode' => 0755,
              'param_name' => 'files',
     ...
     protected function initialize() {
          ...
          //var_dump($this->get_server_var('REQUEST_METHOD'));
     ...
     protected function set_additional_file_properties($file) {
          ...
          $file->patchType = $this->options['patch_type'];
          if ($file->deleteType !== 'DELETE') {
               $file->deleteUrl .= '&_method=DELETE';
          }
          if ($file->patchType !== 'PATCH') {
               $file->deleteUrl .= '&_method=PATCH';
          }
     ...
     protected function get_scaled_image_file_paths($file_name, $version) {
          $file_path = $this->get_upload_path($file_name);
          //$file_path = $this->get_download_url($file_name);
          if (!empty($version)) {
     ...
     protected function gd_imageflip($image, $mode) {
          ...
          $backgroundColorJL = imagecolorallocate($new_img, 255, 255, 255);
          imagefill($new_img, 0, 0, $backgroundColorJL);
          $src_x = 0;
          $src_y = 0;
          switch ($mode) {
     ...
     protected function gd_create_scaled_image($file_name, $version, $options) {
          ...
          if ($scale >= 1 && empty($options['force_resizing'])) {
               if ($image_oriented) {
          ...
          if ($this->options['recadrage_type'] == 'canvas') {
               if (($img_width / $img_height) >= ($max_width / $max_height)) {
                    // Si l'image source a un ratio hauteur/largeur supérieure à la destination
               ...
               // Si elle plus verticale que la destination
               else {
               ...
               if ($this->options['bg_color'] == "black") {
                    $backgroundColorJL = imagecolorallocate($new_img, 0, 0, 0);
               } else {
                    $backgroundColorJL = imagecolorallocate($new_img, 255, 255, 255);
               }
               imagefill($new_img, 0, 0, $backgroundColorJL);
     ...
     protected function imagick_create_scaled_image($file_name, $version, $options) {
          ...
          // Si l'image ne doit pas être orienté
          // ou que le ratio
          // la taille maximale
          if (!($image_oriented || $max_width < $img_width || $max_height < $img_height) && empty($options['force_resizing'])) {
               if ($file_path !== $new_file_path) {
                    return copy($file_path, $new_file_path);
               }
               return true;
          ...
          if ($this->options['recadrage_type'] == 'crop') {
               $x = 0;
               $y = 0;
               if (($img_width / $img_height) >= ($max_width / $max_height)) {
     ...
     protected function handle_image_file($file_path, $file) {
          ...
          foreach ($this->options['image_versions'] as $version => $options) {
               // echo '<br /><pre>'; echo $version; var_dump($options);
     ...
     public function get($print_response = true) {
          ...
          if ($file_name) {
               ...
               //Ajouter ici la recheche par la suite 
               $response = array(
               ...
               if (isset($this->options['sort'])) {
                    switch ($this->options['sort']) {
                         case 'ID_ASC':
                              usort($response[$this->options['param_name']], function($a, $b) {
                                   if (!isset($b->id)) {
                                        return (-1) - $a->id;
                                   } else if (!isset($a->id)) {
                                        return (-1) - $b->id;
               ...
               if (isset($this->options['recherche'])) {
                    $tmp_response = array();
                    foreach ($response[$this->options['param_name']] as $file) {
                         if (strstr(strtolower($file->name), strtolower($this->options['recherche'])) != FALSE) {
                              //var_dump($file->name);
                              $tmp_response[] = $file;
                         }
                    }

                    $response[$this->options['param_name']] = $tmp_response;
                    //var_dump(count($response[$this->options['param_name']]));
               }

               $debut = $this->options['page'] * $this->options['nb_max'] + $this->options['page'];
               $nb_max = (($debut + $this->options['nb_max']) < count($response)) ? count($response) - $debut : $this->options['nb_max'];
               $response[$this->options['param_name']] = array_slice($response[$this->options['param_name']], $debut, $nb_max);
          }
     ...
     public function patch($print_response = true) {
          $file_names = $this->get_file_names_params();
          if (empty($file_names)) {
               $file_names = array($this->get_file_name_param());
          }
